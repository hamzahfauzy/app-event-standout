...
class Tripay
{
	...
	*/
    ...
    {
        $env = config('TRIPAY_ENV');
        $url = $this->URL_channelMs;
        if($env == 'LIVE')
        {
            $url = $this->URL_channelMp;
        }

        $channels = $this->curlAPI($url,'','GET');
    ...
    {
        $env = config('TRIPAY_ENV');
        $url = $this->URL_transMs;
        if($env == 'LIVE')
        {
            $url = $this->URL_transMp;
        }

        return $this->curlAPI($url,$data,'POST');
    }
}